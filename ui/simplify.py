...
def render() -> None:
    ...
    if st.button("✨ Simplify", type="primary", disabled=not (api_key_configured() or showcase)):
        try:
            if showcase:
                showcase_badge(); out = showcase["simplify_summary"]
        ...
        st.success(f"Document type: {out.get('document_type', 'unknown')} · grounded citations: {rate:.0%}") if rate >= .8 else st.warning(f"Grounded citations: {rate:.0%}. Review flagged sections carefully.")
        for section in sections:
            with st.container(border=True):
                st.write(f"**{section.get('original_heading', 'Section')}**")
                st.write(section.get("plain_text", ""))
                if not section.get("grounded"): st.caption("⚠️ Citation needs review.")
                cite(section.get("source_span"), doc)
        summary = "\n\n".join(s.get("plain_text", "") for s in sections)
        if summary:
            st.write(f"**🔊 Audio summary ({out.get('language', language)})**")
            render_speech_button(summary, language)
        if out.get("key_terms"):
            st.write("#### Key terms")
            for term in out["key_terms"]: st.write(f"- **{term.get('term', '')}** — {term.get('meaning', '')}")
        ...
        try:
            ...
            st.download_button("⬇️ Export simplified (PDF)", build_simplified_pdf(out.get("document_type", "legal document"), level, sections), "lexiclarity_simplified.pdf", "application/pdf")
        except (ImportError, ValueError, TypeError) as e: st.caption(f"PDF export unavailable ({e}).")
