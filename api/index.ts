...
export default async function handler(req: AnyRequest, res: AnyResponse) {
  try {
    ...
    if (route === '/simplify') { const text = String(body.document_text || ''); const level = String(body.reading_level || 'summary'); const language = String(body.target_language || 'English'); const levelRules = level === 'simple' ? 'Use everyday adult language, short sentences, and explain legal terms.' : level === 'simpler' ? 'Use very short child-friendly sentences, simple vocabulary, and brief explanations as if explaining to a 10-year-old.' : 'Write a concise executive summary with no more than 5 sections covering parties, money, duration, termination, and the riskiest obligation.'; let result = await askGemini(`Simplify this legal document into valid JSON with document_type, language, reading_level, sections, and key_terms. The selected reading level is ${level}. ${levelRules} The requested output language is ${language}. You MUST write every explanatory field in ${language}. Keep original_heading and source_span exactly as written in the source document. Do not answer in English unless ${language} is English.`, `reading_level: ${level}\ntarget_language: ${language}\ndocument_text:\n${text.slice(0, 50000)}`); if (result && language !== 'English') { result = await askGemini(`Translate this JSON into ${language}. Preserve the selected reading level ${level}; do not expand or shorten the content. Return JSON only.`, JSON.stringify(result)) || result; } return send(res, 200, result ? { ...result, language, reading_level: level } : fallbackSimplify(text, level, language)); }
