...
export default async function handler(req: AnyRequest, res: AnyResponse) {
  try {
    ...
    if (route === '/simplify') { const text = String(body.document_text || ''); const result = await askGemini('Simplify this legal document into valid JSON with document_type, language, reading_level, sections, and key_terms.', `reading_level: ${body.reading_level || 'summary'}\ntarget_language: ${body.target_language || 'English'}\ndocument_text:\n${text.slice(0, 50000)}`); return send(res, 200, result || fallbackSimplify(text, body.reading_level || 'summary', body.target_language || 'English')); }
