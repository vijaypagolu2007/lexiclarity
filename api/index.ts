...
export default async function handler(req: AnyRequest, res: AnyResponse) {
  try {
    ...
    if (route === '/simplify') { const text = String(body.document_text || ''); const language = String(body.target_language || 'English'); let result = await askGemini(`Simplify this legal document into valid JSON with document_type, language, reading_level, sections, and key_terms. The requested output language is ${language}. You MUST write every explanatory field, including plain_text, meaning, summaries, and document_type, in ${language}. Keep original_heading and source_span exactly as written in the source document. Do not answer in English unless ${language} is English.`, `reading_level: ${body.reading_level || 'summary'}\ntarget_language: ${language}\ndocument_text:\n${text.slice(0, 50000)}`); if (result && language !== 'English') { result = await askGemini(`Translate this JSON into ${language}. Translate every explanatory value, including document_type, plain_text, meaning, and summaries. Preserve JSON keys, section IDs, source_span text, and original_heading text exactly. Return JSON only.`, JSON.stringify(result)) || result; } return send(res, 200, result ? { ...result, language } : fallbackSimplify(text, body.reading_level || 'summary', language)); }
